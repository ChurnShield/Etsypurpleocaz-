#!/usr/bin/env python3
"""
Dog Training & Puppy School Design System
Palette: Navy blue #1B3A5C, warm gold #C9A96E, cream #F5F0E8, charcoal #1A1A1A
"""
...
# ── Palette ─────────────────────────────────────────────────────────────────────
NAVY       = (27, 58, 92)      # #1B3A5C — headers, accents
GOLD       = (201, 169, 110)   # #C9A96E — dividers, highlights
CREAM      = (245, 240, 232)   # #F5F0E8 — backgrounds
CHARCOAL   = (26, 26, 26)      # #1A1A1A — body text
WHITE      = (255, 255, 255)
NAVY_DARK  = (15, 35, 58)      # deeper navy
...
def navy_bar(draw, y, h, x0=0, x1=None, canvas_w=None):
    if x1 is None:
        x1 = canvas_w or draw.im.size[0]
    draw.rectangle([x0, y, x1, y + h], fill=NAVY)
...
def field_line(draw, x, y, label, width=2240, font_size=36):
    ...
    y += 58
    draw.rectangle([x, y, x + width, y + 3], fill=NAVY)
    return y + 52
...
def field_pair(draw, x, y, label1, label2, total_w=2240, font_size=36):
    ...
    y2 = y + 58
    draw.rectangle([x, y2, x + half, y2 + 3], fill=NAVY)
    draw.rectangle([x + half + 60, y2, x + total_w, y2 + 3], fill=NAVY)
    return y2 + 52
...
def field_triple(draw, x, y, labels, total_w=2240, font_size=34):
    ...
    for i, lbl in enumerate(labels[:3]):
        ...
        y2 = y + 54
        draw.rectangle([xoff, y2, xoff + w3, y2 + 3], fill=NAVY)
    return y + 54 + 50


def checkbox(draw, x, y, label, font_size=34):
    draw.rectangle([x, y + 4, x + 36, y + 40], outline=NAVY, width=3)
...
def table_row(draw, x, y, cols, widths, row_h=60, alt=False, header=False):
    bg = NAVY if header else (CREAM_ALT if alt else WHITE)
...
def a4_header(img, draw, title, subtitle="Dog Training & Puppy School"):
    W = img.width
...
def a4_footer(draw, canvas_w, canvas_h, template_name=""):
    draw.rectangle([0, canvas_h - 100, canvas_w, canvas_h], fill=NAVY)
