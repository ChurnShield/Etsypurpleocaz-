#!/usr/bin/env python3
"""Generate a Car Detailing Price List PDF.

A4 portrait, white background, full-width photo, serif title,
3 boxed pricing sections with red borders, dark footer bar.
"""
...
PHOTO_PATH = os.path.join(
    ...
    'car_detail_price_list', 'photo_1.jpg'
)
OUTPUT_DIR = os.path.join(
    os.path.dirname(__file__), '..', 'outputs', 'car-detail-price-list'
)
OUTPUT_PATH = os.path.join(OUTPUT_DIR, 'Car_Detail_Price_List.pdf')
...
RED = HexColor('#E02020')
FOOTER_BG = HexColor('#0D0D0D')

PRICING = [
    {
        'title': 'EXTERIOR DETAILING',
        'items': [
            ('Basic wash & dry', 'from \u00a330'),
            ('Full exterior detail', 'from \u00a380'),
    ...
    },
    {
        ...
        'items': [
            ('Interior vacuum & wipe', 'from \u00a340'),
            ('Full interior detail', 'from \u00a390'),
            ('Leather treatment', 'from \u00a360'),
        ],
    },
    {
        'title': 'FULL PACKAGES',
        'items': [
            ('Bronze package', 'from \u00a360'),
            ('Silver package', 'from \u00a3120'),
            ('Gold package', 'from \u00a3200'),
        ],
    },
]
...
def build_pdf():
    ...
    c.setTitle('Car Detailing Price List')
    ...
    c.setFont('Helvetica', 7)
    c.drawRightString(PAGE_W - margin, top_bar_y, 'C A R   D E T A I L I N G   S T U D I O')
    ...
    c.drawCentredString(PAGE_W / 2, title_y, 'CAR DETAILING')
    ...
    for i, sec in enumerate(PRICING):
