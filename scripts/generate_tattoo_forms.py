...
Palette: white background, oxblood #8B1A1A headers, gold #C9A96E dividers.
...
def get_styles():
    ...
    styles.add(ParagraphStyle(
        ...
        textColor=OXBLOOD, spaceAfter=2, spaceBefore=0, alignment=TA_CENTER
    ))
    styles.add(ParagraphStyle(
    ...
    styles.add(ParagraphStyle(
        ...
        textColor=OXBLOOD, spaceBefore=10, spaceAfter=3
    ))
    styles.add(ParagraphStyle(
...
def form_03_aftercare():
    ...
    for t in [
        ...
        story.append(Paragraph(f"<font color='#8B1A1A'>&times;</font>  {t}", s["Body"]))
    ...
    for t in [
        ...
        story.append(Paragraph(f"<font color='#8B1A1A'><b>!</b></font>  {t}", s["Body"]))
...
def main():
    ...
    for old in ["06_Appointment_Card.pdf", "07_Loyalty_Card.pdf"]:
