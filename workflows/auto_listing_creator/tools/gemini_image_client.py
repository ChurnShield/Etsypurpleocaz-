...
PROMPT_TEMPLATES = {
    "appointment card": {
        "card_description": (
            ...
            "each with a SPLIT DESIGN: the left ~38% is metallic GOLD FOIL "
            "texture (crinkled, warm #D4AF77 base with highlight and shadow "
            "variations), and the right ~62% is SOLID BLACK (#0F0F0F). "
            "The two halves are separated by an IRREGULAR TORN-PAPER EDGE — "
            "a jagged diagonal line that looks like ripped paper, running "
            "roughly vertically but slightly diagonal with random tears. "
            "BOTH cards must share the EXACT SAME split layout and colour "
            "scheme — gold foil left, black right, torn edge between them. "
            "BOTH cards use the SAME elegant script font (like Great Vibes "
            "or Brush Script) at the SAME size for their titles. "
            "Front card (right/black side): Title 'Appointment Card' in "
            "white flowing script font. Below it, three form fields in "
            "clean white sans-serif uppercase labels: "
            "NAME: followed by a white rounded rectangle fill-in field. "
            "DATE: followed by a white rounded rectangle fill-in field. "
            "TIME: followed by a white rounded rectangle fill-in field. "
            "Below the fields, a row of six small white rounded day-of-week "
            "buttons: MON  TUE  WED  THU  FRI  SAT. "
            "Back card (right/black side): A circular LOGO placeholder "
            "(white circle outline, ~150px, with 'LOGO HERE' text inside) "
            "centered near the top. Below it, 'Book Appointment' in the "
            "same white script font as the front card. Below that, three "
            "contact lines centered in white sans-serif: "
            "name@yourbusiness.com, 555-555-5555, WWW.YOURBUSINESS.COM. "
            "The gold foil side on BOTH cards is purely decorative texture — "
            "no text or fields on the gold portion"
        ),
        "card_count": "two cards",
        "design_element": (
            "metallic gold foil texture on the left split, irregular torn-"
            "paper edge divider, and fine-line tattoo-style ornamental "
            "accents (thin mandala lines, geometric dot-work)"
        ),
    },
    "gift certificate": {
        "card_description": (
...
def build_product_prompt(product_type, niche, theme,
    ...
    return (
        ...
        # === CARD DESIGN STYLE ===
        f"Card design style: each card has a SPLIT layout — the left ~38% "
        f"is metallic GOLD FOIL texture (crinkled, warm #D4AF77 base with "
        f"realistic highlights and shadows), and the right ~62% is SOLID "
        f"BLACK (#0F0F0F). The two halves are separated by an irregular "
        f"TORN-PAPER edge — a jagged vertical line that looks like ripped "
        f"paper with random tears and fibres visible at the boundary. "
        f"All text and form fields appear on the BLACK (right) side only. "
        f"The GOLD FOIL side is purely decorative with no text. "
        f"Use niche-appropriate ornamental accents on the black side — for "
        f"{niche} this means fine-line art such as thin mandala lines, "
        f"geometric dot-work, or delicate dagger/arrow line art. "
